...
contract NFTAuctionMarket is ERC721URIStorage, ERC2981 {
    ...
    mapping(uint256 => bool) public onAuctionNFTs; //map if nft is either being auctioned or not
    ...
    function createAuctionObject(string memory tokenURI) public returns (uint256) {
        ...
        //fees are expressed in basis points so: 200 = 2% 
        _setTokenRoyalty(tokenCounter, msg.sender, 200);
    ...
    function endAuction(uint256 _auctionID) public {
        ...
        address creator;
        Auction storage auction = auctions[_auctionID];
        (creator,) = royaltyInfo(auction.nftId, auction.highestBid);
        (,royaltyAmount) = royaltyInfo(auction.nftId, auction.highestBid);
        ...
        auction.ended = true;
        // Valid for both if statements: if there is a highest bidder, transfer their bid to the seller

        // if auction seller is not the nft creator/author
        if (auction.highestBidder != address(0) && auction.seller != creator) {
        ...
        // if auction seller is the original nft creator
